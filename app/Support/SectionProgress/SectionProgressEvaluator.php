...
namespace App\Support\SectionProgress;
...
class SectionProgressEvaluator
{
    ...
    public static function courseOfferingRelations(): array
    {
        return [
     ...
     */
    public static function evaluateEnrollment(CourseEnrollment $enrollment): array
    {
        $courseOffering = $enrollment->courseOffering;

        if (! $courseOffering instanceof CourseOffering) {
            return self::evaluateNotEvaluableProgress('No Section Progress Basis', 'No section progress evaluation is available because this enrollment is not linked to a course offering.');
        }

        $courseOffering->loadMissing(self::courseOfferingRelations());
        ...
        return match ($courseOffering->progress_basis) {
    ...
    public static function formatProgressBasisLabel(?string $value): string
    {
        return CourseOffering::PROGRESS_BASIS_OPTIONS[$value] ?? self::formatGenericStatusLabel($value);
    }

    public static function formatProgressStatusLabel(string $value): string
    {
        return self::PROGRESS_STATUS_LABELS[$value] ?? self::formatGenericStatusLabel($value);
    }

    public static function progressStatusBadgeClasses(string $value): string
    {
        return self::PROGRESS_STATUS_BADGE_CLASSES[$value] ?? self::PROGRESS_STATUS_BADGE_CLASSES['not_evaluable'];
    }

    public static function formatGenericStatusLabel(?string $value): string
    {
        if (blank($value)) {
            return '—';
        }
    ...
    public static function formatDeliveryModeLabel(?string $value): string
    {
